        ...
        <el-form ref="newProjectRuleForm" :model="paramsNewProjects" :rules="taskTules">
          ...
          <el-row>
            <el-col :span="12">
              ...
              <el-form-item label="检测开始日期：" class="dialog-form-item" prop="testing_time" :size="size">
                <el-date-picker
              ...
              <el-form-item label="检测结束时间：" class="dialog-form-item" prop="testing_completion_time" :size="size">
                <el-date-picker
              ...
              <el-form-item label="供电变压器容量概况：" class="dialog-form-item" prop="transformer_capacity" :size="size">
                <el-input v-model="paramsNewProjects.transformer_capacity" class="dialog-form-item" type="text" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="建筑总面积（㎡）：" class="dialog-form-item" prop="building_total_area" :size="size">
                <el-input v-model="paramsNewProjects.building_total_area" class="dialog-form-item" type="text" />
              </el-form-item>
              <el-form-item label="建筑层数：" class="dialog-form-item" prop="building_floors" :size="size">
                <el-input v-model="paramsNewProjects.building_floors" class="dialog-form-item" type="text" />
              </el-form-item>
              <el-form-item label="建筑高度：" class="dialog-form-item" prop="building_height" :size="size">
              ...
              <el-form-item label="环境温度（℃）：" class="dialog-form-item" prop="testing_temperature" :size="size">
                <el-input v-model="paramsNewProjects.testing_temperature" class="dialog-form-item" oninput="value=value.replace(/[^\d.]/g,'')" type="text" />
              </el-form-item>
              <el-form-item label="环境湿度（%）：" class="dialog-form-item" prop="testing_temperature" :size="size">
                <el-input v-model="paramsNewProjects.testing_humidity" class="dialog-form-item" oninput="value=value.replace(/[^\d.]/g,'')" type="text" />
              </el-form-item>
              <el-form-item label="检测内容：" class="dialog-form-item" prop="testing_id_list" :size="size">
                <el-select v-model="paramsNewProjects.testing_id_list" multiple filterable placeholder="请选择检测类型" style="width: 100%">
                  <el-option
                    v-for="item in testingContentOptions"
                    :key="item.type_id"
                    :label="item.type_name"
                    :value="item.type_id"
                  />
...
<script>
import { getProject, newProject, detailProject, updateProject, deleteProject, generateReport, exportTemp, importTemp } from '@/api/project3s'
...
export default {
  ...
  data() {
    return {
      ...
      testingContentOptions: [
        { type_id: 3, type_name: '变配电装置' },
        { type_id: 4, type_name: '低压配电线路' },
        { type_id: 5, type_name: '照明装置和常用电器装置' },
        { type_id: 6, type_name: '电气火灾监控系统' }
      ],
      pos: {
        sname: '', // 地点名称
        slat: 0, // 纬度
      ...
      taskTules: {
        ...
        building_total_area: [{ required: true, message: '请填写建筑面积', trigger: 'blur' }],
        building_height: [{ required: true, message: '请填写建筑高度', trigger: 'blur' }],
        building_floors: [{ required: true, message: '请填写建筑层数', trigger: 'blur' }],
        ...
        testing_id_list: [{ required: true, message: '请选择检测内容', trigger: 'change' }],
        testing_basis: [{ required: true, message: '请填写检测依据', trigger: 'blur' }],
        testing_type_id: [{ required: true, message: '请选择检测类型', trigger: 'change' }],
        testing_temperature: [{ required: true, message: '请填写环境温度（℃）', trigger: 'blur' }],
        testing_humidity: [{ required: true, message: '请填写环境湿度（%）', trigger: 'blur' }],
        testing_part_function: [{ required: true, message: '请填写测试部分使用功能', trigger: 'blur' }],

        transformer_capacity: [{ required: true, message: '请填写供电变压器容量概况', trigger: 'blur' }],
      ...
      paramsNewProjects: {
        ...
        building_total_area: undefined, // 建筑面积(平方米)
        building_height: undefined,		// 建筑高度(米)

        testing_part: undefined,
        testing_area: undefined,
        testing_part_function: undefined,
        testing_type_id: undefined,
        testing_temperature: undefined,
        testing_humidity: undefined,
        testing_time: undefined,
        testing_completion_time: undefined,
        testing_basis: undefined,
        testing_id_list: undefined,

        leader: undefined,			// 项目负责人
        auditor: undefined,		// 审核人
        testing_users: undefined		// 检测人
      }
    }
  },
  computed: {
  ...
  methods: {
    ...
    UploadFile(content) {
      ...
      importTemp(formData).then((res) => {
        ...
        for (const key in data) {
          if (['testing_time', 'testing_completion_time'].includes(key)) {
    ...
    exportTemp() {
      exportTemp({ project_id: this.project_id }).then(res => {
        ...
        const fileName = (this.paramsNewProjects.name || '安全检测') + '.xlsx'
    ...
    openEditProject(project_id) {
      ...
      detailProject({ project_id }).then(res => {
        ...
        for (const key in data) {
          ...
          if (key === 'testing_id_list') {
            obj[key] = data[key].split(',').map(item => parseInt(item))
          }
        }
