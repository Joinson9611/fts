<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <!--新建任务-->
      <el-button v-waves class="filter-item" style="margin-left: 10px" type="primary" icon="el-icon-plus" @click="openNewDevice">新建设备</el-button>
      <!--删除按钮-->
      <el-button v-waves :disabled="multipleSelection.length===0" class="filter-item" type="danger" icon="el-icon-delete" @click="deleteDevice">删除设备</el-button>
    </div>
    <!--任务列表-->
    <el-table
    ...
    >
      ...
      <el-table-column type="expand">
        <template slot-scope="scope">
          <el-form label-position="left" inline class="demo-table-expand">
            ...
            <el-form-item label="是否符合法定市场准入规则：">
      ...
      <el-table-column label="任务名称" align="center">
        <template slot-scope="scope">
          <a style="color: #409EFF" @click="openEditDevice(scope.row)"><i>{{ scope.row.task_name }}</i></a>
        </template>
      </el-table-column>
    ...
    <el-dialog :visible.sync="dialogVisible" :append-to-body="true" :close-on-click-modal="false" :title="isEdit?'编辑':'新建'" @closed="closeDialog">
      <el-form ref="formDevice" :model="paramsDeviceInfo" :rules="deviceInfoRules" label-width="120px">
        ...
        <el-form-item label="生产日期：" class="dialog-form-item" prop="product_time">
          <el-date-picker
...
export default {
  ...
  methods: {
    ...
    openEditDevice(info) {
      ...
      this.paramsDeviceInfo.product_time = Formattimestamp(this.paramsDeviceInfo.product_time)
    },
    // 任务列表改变
    taskChange(id) {
      this.task_id = id
